...
# Configuración visual de página
...
# 1. Función de consulta a LLM vía Groq con filtro de modelos de texto y formato estricto
def consultar_llama3(prompt_sistema, prompt_usuario):
    ...
    try:
        ...
        st.error(f"Error con los modelos disponibles: {ultimo_error}")
        return None
...
if archivo_cargado is not None:
    ...
    with st.spinner("Procesando datos y calibrando variables..."):
        ...
        cols_num = [
            "UNDERLYING_LAST", "DTE", "C_IV", "C_VOLUME", "C_LAST", 
            "C_BID", "C_ASK", "STRIKE", "PREMIUM", "PNL_PER_SHARE", "PROBABILIDAD_COMPRA"
        ]
        for c in cols_num:
            if c in df.columns:
        ...
        features = [
            "UNDERLYING_LAST", "STRIKE", "PREMIUM", "DTE", "C_IV", 
            "C_VOLUME", "MONEYNESS", "STRIKE_DISTANCE_CALC_PCT", "BID_ASK_SPREAD_PCT"
        ]
    ...
    # 4. Filtros interactivos en barra lateral
    ...
    with tab1:
        ...
        with col_g1:
            ...
            fig_v = px.bar(
                ...
                y="STRIKE", 
                orientation="h", 
                title="Concentración de Liquidez por Strike ($)", 
        ...
        with col_g2:
            ...
            fig_s = px.scatter(
                ...
                y="C_IV", 
                color="SIGNAL_REAL", 
                title="Estructura de Volatilidad (IV) vs Moneyness"
            )
    ...
    with tab2:
        ...
        with col_m2:
            df_filtrado["RANGO_CONF"] = pd.cut(
                df_filtrado["PROBABILIDAD_COMPRA"], 
            ...
            fig_conf = px.bar(
                ...
                x="RANGO_CONF", 
                y="PNL_PER_SHARE", 
                title="Retorno Promedio según Nivel de Confianza ($)", 
                color_discrete_sequence=["#0284c7"]
            )
            ...
            if st.button("Generar Diagnóstico Predictivo"):
                with st.spinner("Consultando Llama 3..."):
                    prompt_s = (
                        ...
                        "No expliques definiciones básicas como TP o TN. Ve directo al valor operativo. "
